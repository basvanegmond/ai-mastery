...
export function DomainCard({ domain, currentScore, activityThisWeek }: DomainCardProps): JSX.Element {
  ...
  return (
    <div className="rounded-xl bg-canvas border border-edge p-4">
      <div className="flex items-start justify-between gap-2 mb-3">
        <div className="min-w-0">
          <h3 className="text-[14px] font-semibold leading-snug text-ink truncate">
            {domain.name}
          </h3>
      ...
      <div className="flex justify-between mt-1">
        {TICKS.map((n) => (
          <span key={n} className="text-[9px] text-ink-sub opacity-50 leading-none">
            {n}
          </span>
        ))}
      </div>
